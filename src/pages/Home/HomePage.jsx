...
export default function HomePage() {
	...
	useEffect(() => {
		const addPost = (event) =>
			setPosts((current) => [event.detail, ...current]);
		...
		const timer = window.setTimeout(async () => {
			...
			try {
				if (isDemo) {
					...
					setSearchPosts(
						getDemoPosts().filter((post) =>
							post.content?.toLowerCase().includes(value),
						),
					);
				} else {
					const [people, foundPosts] = await Promise.all([
						usersService.search(search),
						postsService.search(search),
					]);
					setUsers(people);
	...
	return (
		<div className="feed-page page-column">
			<header className="page-heading">
				<div>
					<span className="eyebrow">
						{isDiscover ? "A little farther out" : "Your conversations"}
					</span>
					<h1>{isDiscover ? "Discover" : "Good morning."}</h1>
					<p>
						{isDiscover
							? "Find people and conversations worth a detour."
							: "What's moving through your world today?"}
					</p>
				</div>
				<span className="page-heading__mark">
					<Sparkles size={20} />
				</span>
			</header>
			{!isDiscover && <Composer />}
				...
				<section className="discover-search">
					<div className="discover-search__field">
						...
						<input
							...
							placeholder="Search TTM"
							autoFocus
						/>
					</div>
					<div className="discover-search__tabs">
						<button
							className={searchTab === "people" ? "is-active" : ""}
						...
						<button
							className={searchTab === "posts" ? "is-active" : ""}
			...
			{isLoading || searching ? (
				...
				<EmptyState
					...
					text={error}
					action={
						<button className="button button--secondary" onClick={loadPosts}>
							Try again
						</button>
					}
				/>
			) : isDiscover && !search.trim() ? (
				<EmptyState
					icon={Search}
					title="Find your next conversation"
					text="Search by name, username, or words inside a post."
				/>
			) : isDiscover && searchTab === "people" ? (
				...
				<EmptyState
					...
					title={isDiscover ? "No posts found" : "It's quiet here for now"}
					text={
						isDiscover
				...
				<div
					className={`feed-list ${isDiscover ? "" : "feed-list--divided"}`.trim()}
				>
