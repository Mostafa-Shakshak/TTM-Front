...
export default function NotificationsPage() {
	...
	useEffect(() => {
		if (latestNotification)
			setNotifications((current) =>
				current.some((item) => item.id === latestNotification.id)
					? current
					: [latestNotification, ...current],
			);
	}, [latestNotification]);
	...
	async function respond(request, action) {
		...
		try {
			if (!isDemo) {
				if (action === "accept") await followService.accept(request.id);
				else await followService.reject(request.id);
			}
			setRequests((current) =>
	...
	async function loadMore() {
		...
		try {
			const data = await notificationsService.getAll(next);
	...
	return (
		<div className="notifications-page page-column">
			<header className="page-heading page-heading--compact">
				<div>
					<span className="eyebrow">Your people</span>
					<h1>Activity</h1>
					<p>
						Requests, replies, and moments that found their way back to you.
					</p>
				</div>
				...
				<>
					<section className="request-section">
						<div className="section-title">
							...
							{requests.length > 0 && <span>{requests.length} new</span>}
						</div>
						{requests.length === 0 ? (
							<p className="section-empty">No pending follow requests.</p>
						) : (
							<div className="request-list">
								{requests.map((request) => (
									<article className="request-item" key={request.id}>
										<Avatar user={request.follower} size="md" />
										<div>
					...
					<section className="activity-section">
						...
						{notifications.length === 0 ? (
							<EmptyState
								...
								text="New likes, comments, and follows will appear here."
							/>
						) : (
							<div className="notification-list">
								...
								{pagination.page < pagination.pages && (
									<button
										className="button button--quiet notification-list__more"
										onClick={loadMore}
									>
										Load more
									</button>
